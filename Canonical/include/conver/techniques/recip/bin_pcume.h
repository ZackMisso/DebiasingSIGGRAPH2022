...
template <typename T>
struct PCumeRecipBin : Technique<T>
{
    PCumeRecipBin(Float maj) : maj(maj)
    {
        // blah
    }
    ...
    virtual T eval_problem(const Problem<T> *problem, int samples, int &cost, pcg32 &rng) const
    {
        ...
        while (true)
        {
            ...
            sum += weight;
            Float pos1 = problem->getBounds()->uniformSample(rng);
            cost++;

            weight *= (tau - problem->eval_integral(pos1)) / tau;
    ...
    virtual std::string getName() const { return "pcume_bin"; }
