...
struct PhotonAlphaCone : Technique<Float>
{
    struct Photon
    {
    ...
    PhotonAlphaCone(Float alpha = 0.9,
                    Float c = 0.0,
                    Float base_r = 1.0,
    ...
    virtual Float eval_problem(const Problem<Float> *problem, int samples, int &cost, pcg32 &rng) const
    {
        ...
        if (alpha_pmf == 0.0)
            alpha_pmf = 1.0; // n_1 and n_2 == so this is fine
    ...
    virtual std::string getName() const { return "photon_alpha_cone"; }
