...
struct PhotonSquareQuart : Technique<Float>
{
    struct Photon
    {
    ...
    PhotonSquareQuart(Float r = 0.65,
                     Float base_r = 1.0,
    ...
    virtual Float eval_problem(const Problem<Float> *problem, int samples, int &cost, pcg32 &rng) const
    {
        ...
        int total_photons = base_photons * pow(2.7, Float(n));
        ...
        for (int l = 0; l < base_samples; ++l)
        {
            ...
            for (int i = 0; i < total_photons; ++i)
            {
                ...
                if (sqr_dist <= base_r * base_r)
                    base += energy * (1.0 - sqr_dist / (base_r * base_r))* (1.0 - sqr_dist / (base_r * base_r));
                if (sqr_dist <= r * r)
                    p += energy * (1.0 - sqr_dist / (r * r))* (1.0 - sqr_dist / (r * r));
                if (sqr_dist <= r1 * r1)
                    p1 += energy * (1.0 - sqr_dist / (r1 * r1))* (1.0 - sqr_dist / (r1 * r1));
            }
    ...
    virtual std::string getName() const { return "photon_square_quart"; }
