...
template <typename T>
struct NvidiaPink : Technique<T>
{
    NvidiaPink(Float gl_maj, Float C, int m) : gl_maj(gl_maj), C(C), m(m)
    {
        // blah
    }

    T pink(Float tau) const
    {
        return std::pow(1.0 + tau * C * C, -1.0 / (C * C));
    }

    virtual T eval_problem(const Problem<T> *problem, int samples, int &cost, pcg32 &rng) const
    {
        // Float mult = (1.0) / (C * C * C * C);
        Float mult = 1.0;
        ...
        for (int i = 0; i <= k; ++i)
        {
            ...
            for (int j = 0; j <= k; ++j)
            {
                ...
                if (j != 0)
                {
                    c_fact *= (1.0 + double(j - 1) * C * C);
                    c_fact /= (1.0 + X_j * C * C);
                }
                sum += means[j] * k_fact * c_fact * rr_weights[j];
            }

            trans += 1.0 / double(k + 1) * pink(X_j) * sum;
        }
    ...
    double expected_order(double c, double p_z) const
    {
        ...
        for (int k = 1; k <= K; ++k)
        {
            prod *= (1.0 + double(k - 1) * C * C);
            t = t * c / double(k) * prod;
            sum += t;
        }

        double E_n = K + (pink(c) - sum) / t;
        ...
        int &k) const
    {
        ...
        while (true)
        {
            double c_i = fmin(c * (1.0 + C * C * (i - 1.0)) / i, 1.0);
    ...
    virtual std::string getName() const { return "nvidia_pink_" + std::to_string(m); }
