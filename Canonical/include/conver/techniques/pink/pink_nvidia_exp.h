...
template <typename T>
struct NvidiaPinkExp : Technique<T>
{
    NvidiaPinkExp(Float gl_maj, Float C) : gl_maj(gl_maj), C(C)
    {
        // blah
    }
    ...
    virtual T eval_problem(const Problem<T> *problem, int samples, int &cost, pcg32 &rng) const
    {
        ...
        for (int i = 0; i <= k; ++i)
        {
            ...
            for (int j = 0; j < m; ++j)
            {
                ...
                x_i += problem->eval_function(loc) * mult;
                cost++;
            }
    ...
    virtual std::string getName() const { return "nvidia_pink_exp"; }
