...
void ReciprocalHeatPlots::run_experiment_noise(
    pcg32 &rng,
    ...
    std::string prefix) const
{
    ...
    CompositeSineFunction comp_func = CompositeSineFunction(rng, 30, 0.0, 1.0);
    comp_func.normalize();
    ...
    for (double p = min_p; p < max_p; p += (max_p - min_p) / Float(res))
    {
        ...
        for (double yx = min_yx; yx < max_yx; yx += (max_yx - min_yx) / Float(res))
        {
            ...
            Reciprocal recip = Reciprocal(&comp_iter_func);
            ...
            for (int k = 0; k < techniques.size(); ++k)
            {
                ...
                images[5 * k + 2]->setPixel(j, i, imedit::Pixel(avg_data.getRecordByIndex(k).data_points[data_pts - 1].rmse));
            ...
            mean_image.setPixel(j, i, imedit::Pixel(1.0 / comp_iter_func.integrate()));
...
void ReciprocalHeatPlots::run_experiment_const(
    pcg32 &rng,
    std::vector<Technique<Float> *> techniques,
    std::string prefix) const
{
...
void ReciprocalHeatPlots::run_experiment_scos(
    pcg32 &rng,
    std::vector<Technique<Float> *> techniques,
    std::string prefix) const
{
...
void ReciprocalHeatPlots::run_experiment_gauss(
    pcg32 &rng,
    std::vector<Technique<Float> *> techniques,
    std::string prefix) const
{
...
void ReciprocalHeatPlots::run() const
{
    ...
    PCumeRecip<Float> *pcume = new PCumeRecip<Float>(1.0);
    PCmfRecip<Float> *pcmf = new PCmfRecip<Float>(1.0, 0.1);
    PResRatioRecip<Float> *prat = new PResRatioRecip<Float>(1.0, 0.1);
    BinNonBinBernoulli<Float> *bern = new BinNonBinBernoulli<Float>(1.0);
...
void ReciprocalHeatPlots::run(int test) const
{
